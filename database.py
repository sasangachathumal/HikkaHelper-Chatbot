...
DATABASE_DIR = 'data'
DATABASE_PATH = os.path.join(DATABASE_DIR, 'travel_info.db')
...
def get_info_by_category(category, limit=None, offset=None, exclude_ids=None):
    ...
    for row in results:
        ...
        entry = f"- {row['name']}: {row['description']}" if row['name'] else f"- {row['description']}"
        if row['details']:
            entry += f" (Details: {row['details']})"
        formatted_results.append(entry)

    return {"results": formatted_results, "ids": item_ids}
